...
def test_create_rdv_with_patient():
    """Test de création d'un rendez-vous avec un vrai patient"""
    print("🔍 Test de création d'un rendez-vous avec patient")
    print("=" * 50)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Récupérer les vrais patients (table Patient)
            print("\n📋 Récupération des patients...")
            response = requests.get(f"{BASE_URL}/patients/", headers=headers)
            print(f"Status patients: {response.status_code}")
            ...
            if response.status_code == 200:
                ...
                if isinstance(patients_data, list) and len(patients_data) > 0:
                    ...
                    print(f"✅ Patient sélectionné: {patient.get('user', {}).get('first_name')} {patient.get('user', {}).get('last_name')} (ID Patient: {patient['id']})")
                else:
                    print("❌ Aucun patient trouvé dans la table Patient")
                    return
            else:
            ...
            rdv_data = {
                "patient": patient['id'],  # ID du patient (pas de l'utilisateur)
                "service": service['id'],
                "docteur": doctor['id'],
