...
def test_create_rdv():
    """Test de création d'un rendez-vous avec un patient existant"""
    print("🔍 Test de création d'un rendez-vous")
    print("=" * 50)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Récupérer un patient pour le test
            print("\n📋 Récupération d'un patient...")
            response = requests.get(f"{BASE_URL}/users/?role=patient", headers=headers)
            if response.status_code == 200:
                patients_data = response.json()
                if patients_data.get('results') and len(patients_data['results']) > 0:
                    patient = patients_data['results'][0]
                    print(f"✅ Patient sélectionné: {patient['first_name']} {patient['last_name']} (ID: {patient['id']})")
                else:
