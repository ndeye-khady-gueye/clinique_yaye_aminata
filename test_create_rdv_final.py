...
def test_create_rdv_final():
    """Test final de création d'un rendez-vous"""
    print("🔍 Test final de création d'un rendez-vous")
    print("=" * 50)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Récupérer un utilisateur avec rôle patient
            print("\n📋 Récupération d'un utilisateur patient...")
            response = requests.get(f"{BASE_URL}/users/?role=patient", headers=headers)
            if response.status_code == 200:
                users_data = response.json()
                if users_data.get('results') and len(users_data['results']) > 0:
                    user = users_data['results'][0]
                    print(f"✅ Utilisateur patient sélectionné: {user['first_name']} {user['last_name']} (ID User: {user['id']})")
                else:
                    print("❌ Aucun utilisateur patient trouvé")
                    return
            else:
                print(f"❌ Erreur récupération utilisateurs patients: {response.status_code}")
                return
            
            # Récupérer le profil Patient correspondant
            print("\n📋 Récupération du profil Patient...")
            response = requests.get(f"{BASE_URL}/patients/", headers=headers)
            print(f"Status patients endpoint: {response.status_code}")
            
            if response.status_code == 200:
                patients_data = response.json()
                print(f"Patients data: {patients_data}")
                
                # Chercher le patient correspondant à l'utilisateur
                patient_id = None
                if patients_data.get('results'):
                    for patient in patients_data['results']:
                        if patient.get('user', {}).get('id') == user['id']:
                            patient_id = patient['id']
                            print(f"✅ Profil Patient trouvé: ID {patient_id}")
                            break
                
                if not patient_id:
                    print("❌ Aucun profil Patient trouvé pour cet utilisateur")
                    return
            else:
